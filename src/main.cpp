...
#include "io/sensor_bmp280.h"
#include "io/sensor_bme280.h"
...
void node::setup()
{
    ...
    loader.install_factory("bmp280", 0, [this](const JsonObject &obj, int vnode_id) -> ticker_component*
    {
        int refresh_rate = 10;
        if(obj.containsKey("rate") && obj["rate"].is<int>())
            refresh_rate = obj["rate"].as<int>();
        ...
        sensor_bmp280 *sensor = new sensor_bmp280();
        sensor->begin(refresh_rate*1000, always_notify, altitude);
        sensor->on_temperature_changed = [this, vnode_id](const float value)
        {
            Serial.printf("BMP280 #%d Temperature changed: ", vnode_id);
            Serial.print(value);
            Serial.println(" °C");
            mqtt.publish(get_state_topic("temperature", vnode_id),"local,"+String(value));
        };
        ...
        sensor->on_pressure_changed = [this, vnode_id](const float absolute, const float sea_level)
        {
            Serial.printf("BMP280 #%d Pressure changed: ", vnode_id);
            Serial.print(absolute);
            Serial.print("mbar (sea-level: ");
            Serial.print(sea_level);
            Serial.println("mbar)");
    ...
    loader.install_factory("bme280", 0, [this](const JsonObject &obj, int vnode_id) -> ticker_component*
    {
        int refresh_rate = 10;
        if(obj.containsKey("rate") && obj["rate"].is<int>())
            refresh_rate = obj["rate"].as<int>();
        ...
        sensor->on_humidity_changed = [this, vnode_id](const float value)
        {
            Serial.printf("BME280 #%d Humidity changed: ", vnode_id);
            Serial.print(value);
            Serial.println(" %");
            mqtt.publish(get_state_topic("humidity", vnode_id),"local,"+String(value));
        };

        sensor->on_pressure_changed = [this, vnode_id](const float absolute, const float sea_level)
        {
            Serial.printf("BME280 #%d Pressure changed: ", vnode_id);
            Serial.print(absolute);
            Serial.print("mbar (sea-level: ");
            Serial.print(sea_level);
            Serial.println("mbar)");
