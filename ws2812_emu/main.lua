...
function love.load()
	ws2812.pattern=string.char(0,0,0):rep(ws2812._len)
	love.window.setMode(20*ws2812._len,20)
end

function love.update(dt)
	dtsum=dtsum+dt
	while dtsum > timer_interval do
		update()
		dtsum = dtsum - timer_interval
	end
end
...
function love.keypressed(key, isrepeat)
	...
	if key=='a' then
		fade_strip_to_random_colors()
	elseif key=='u' then
		fade_strip_to_color(math.random(0,255),math.random(0,255),math.random(0,255),0.5)
	elseif key=='i' then
		fade_strip_to_color(255,255,255,0.5)
	elseif key=='o' then
		fade_strip_to_color(0,0,0,0.5)
	elseif key=='s' then
		set_strip_to_color(255,255,255)
	elseif key=='r' then
		animate_strip_pingpong(color(255,0,0),color(0,0,0),0.05)
	elseif key=='q' then
		love.event.quit()
	end
end
